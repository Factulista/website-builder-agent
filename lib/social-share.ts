 ...
 */
export function buildSocialShareBlock(pageUrl: string, pageTitle: string): string {
...
<style>
.fact-share-links{display:flex !important;gap:14px !important;align-items:center !important;justify-content:flex-start !important;margin-top:20px !important;padding:0 !important;width:auto !important;position:static !important;float:none !important}
...
export function injectSocialShareLinks(html: string, pageUrl: string, pageTitle: string): string {
  ...
  const block = buildSocialShareBlock(pageUrl, pageTitle)
