...
- add_page — REGOLA CSS: La nuova pagina DEVE includere un blocco <style> completo con tutto il CSS necessario per i suoi componenti specifici (layout, sezioni, tabelle, card, ecc.). I token di design (:root con colori, font, spaziature) vengono sincronizzati automaticamente dal sito — usa le variabili CSS (var(--accent), var(--bg), var(--text), ecc.) invece di valori hardcoded così i colori del brand saranno coerenti. COPIA navbar e footer dalla home page così sono identici. Genera una pagina HTML completa e stilata, autonoma visivamente.
