...
export const AGENTS_MANIFEST: AgentMeta[] = [
  ...
  },
  {
    ...
    outputs: ['seo edits (solo blog post ora)'],
    systemPromptPreview: 'Legacy. SEO pagine ora tramite tool run_seo_audit nel Master HTML Agent. Attivo solo per aggiornamento SEO blog post (seo_title, seo_description, tags).',
    filePath: 'lib/agents/seo-agent.ts',
    enabled: true,
    rules: [
      '⚠️ Non riceve più chiamate per SEO pagine — gestito da run_seo_audit tool',
      'Attivo solo per blog post: aggiorna seo_title, seo_description, tags in Supabase',
      'Il SEO compiler deterministico (lib/seo-compiler.ts) fa i check: title/H1/desc/img/schema',
      'Score badge (SEO 85✓) visibile nell\'header vicino al bottone Pubblica',
      'Pubblica BLOCCATA se critical issues (H1 mancante, title mancante, ecc.)',
    ],
  },
  {
    name: 'design-update',
  ...
  {
    name: 'content-update',
    displayName: 'Content Update Agent (disabilitato)',
    description: '⚠️ DISABILITATO. Le modifiche ai testi sono gestite direttamente dal Master HTML Agent via edit_block o edit_page. Non più necessario come agente separato.',
    model: 'claude-haiku-4-5-20251001',
    maxTokens: 8192,
    category: 'utility',
    inputs: ['user request', 'pages attuali', 'context'],
    outputs: ['pages con testi aggiornati'],
    systemPromptPreview: 'Disabilitato. Il Master HTML Agent gestisce le modifiche testo con edit_block (block-scoped, Haiku, ~6k tok) invece di riscrivere pagine intere.',
    filePath: 'lib/agents/content-agent.ts',
    enabled: false,
    rules: [
      '⚠️ Agente non più attivo nel flusso principale',
      'Le traduzioni e riscritture testo ora: Master Agent → read_block → edit_block',
      'Vantaggio: context ridotto a 6k tok (solo il blocco) vs 73k (pagina intera)',
    ],
  },
  {
    name: 'site-analyzer',
